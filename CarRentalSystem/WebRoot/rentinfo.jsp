  ...
  <div class="main">
  	...
  	<%
  			if(request.getParameter("del")!=null)
  			{
  				...
  				String sqldel="update cars set rentout=null,renttime=null,state='在库' where carid="+delid;
  				int flag=0;
  				flag=statement.executeUpdate(sqldel);
  				if(flag > 0)
				{%>
					<script type='text/javascript'>alert('取消租车成功！');
					window.location.href='rentinfo.jsp';</script>");
				<%}	
 	 		}
  	 %>
  	<table border="1" rules="all" style="margin:60px auto;">
 	<tr><td colspan="10" style="height:40px;  width:900px;" ></td></tr>
 	<tr><td style="height:40px;">车辆id</td><td>车主</td><td>车牌号</td><td>车型号</td><td>车颜色</td><td>出租价格</td><td>出租日期</td><td>租借方</td><td>在库状态</td><td>停止租入</td></tr> 
 <%          
		String sqlshow = "select * from cars where rentout='"+u_name+"'";            
        ...
        {%>
 			...
 			<td><a href="rentinfo.jsp?del=<%= rsshow.getString("carid")%>" onClick="return confirm('确定删除?');">取消租入</a></td> 
	  		</tr>
  		<%}
  		rsshow.close();
  		conn.close();
